...
- *Afspm Director*: mediates control between components and translator (only 1 component in control at a time) and caches data received.
...
- These are passed on by the AfspmDirector. Data is *stored* in a *cache* and re-sent to new/crashed components.
...
- The AfspmDirector determines which *client* is *in-control*, and *forwards* these to the MicroscopeTranslator.
...
#+beamer:\pause
*** Control Modes
The AfspmDirector defines the *control mode*, which can be:
