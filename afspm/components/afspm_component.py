...
class AfspmComponent(metaclass=ABCMeta):
    """Base class for afspm component.

    This class concerns itself with the setup of an ipc heartbeater, and the
    base run() method. Part of this is the abstract in-loop method
    run_per_loop() (which all child classes will need to extend).
    ...
    for the duration of the experiment. With this, a listener can easily be
    created for every component in an experiment, residing on the same PC.
    Said listener will ensure the component stays alive and, if not,
    respawn the component accordingly.
    ...
    def run_per_loop(self):
        ...
        This is the main method any subclass should implement, in order to
        define what the AfspmComponent does per loop. (Avoid modifying run(),
        as this handles base AfspmComponent logic.)
        """
